...
class SupaPhoneAuth extends StatefulWidget {
  ...
  const SupaPhoneAuth({Key? key, required this.phoneAuthAction, required this.redirectUrl}) : super(key: key);
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (isSigningIn) {
                ...
                try {
                  await supaAuth.createNewPhoneUser(_phone.text, _password.text);
                  if (!mounted) return;
                  await successAlert(context);
                  if (mounted) {
                    Navigator.popAndPushNamed(context, widget.redirectUrl, arguments: {"phone": _phone.text});
